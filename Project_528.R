...
mean(covid_Asia$stringency_index,na.rm = TRUE)
covid_Asia$stringency_index<- ifelse(is.na(covid_Asia$stringency_index),61.04,covid_Asia$stringency_index)
...
ggplot(covid_North_America_filter,aes(x=location,y=count,color=location,fill=location))+geom_col()+scale_y_continuous(name='Flourescent intensity/arbitrary units',labels= scales::comma)+theme_bw()
...
compare<-covid %>% filter(location=='Niger'|location=='United States')%>%group_by(location)%>% summarise(date,total_cases,total_deaths,population,total_tests)
compare$comparable<- compare$total_cases/compare$population
ggplot(compare,aes(x=date,y=comparable,color=location))+geom_line()
...
#########mean bed impact on data############################
library(dplyr)
covid_hospital <- covid %>% select(iso_code, continent, location,population, total_cases_per_million, hospital_beds_per_thousand)


library(visdat)
vis_miss(covid_hospital)

mean(covid_hospital$hospital_beds_per_thousand,na.rm = TRUE)
covid_hospital$hospital_beds_per_thousand<- ifelse(is.na(covid_hospital$hospital_beds_per_thousand),3.097,covid_hospital$hospital_beds_per_thousand)

library(ggplot2)
covid_hospital %>% ggplot(aes(x = hospital_beds_per_thousand, y = total_cases_per_million, colour = iso_code)) + geom_point()




hospital <- covid_hospital %>%filter(location!="World")%>%  group_by(location) %>%
  summarise(mean_cases=mean(total_cases_per_million, na.rm = TRUE),
            mean_beds=mean(hospital_beds_per_thousand,na.rm = TRUE),population,continent)

hospital<-delete.na(hospital,0)
hospital %>% ggplot(aes(x = mean_beds, y = mean_cases,color=continent)) + geom_point()+ labs(x="mean number of beds per case",y="mean cases per day")+theme_bw()

hospital <- hospital[order(hospital$mean_beds),]
head(hospital)
tail(hospital)

hospital %>% filter(location == "Ukraine"|location=="Belarus"|location=="South Korea"|location=="Japan"|location=="Monaco"|location == "Mali"|location=="Madagascar"|location=="Ethiopia"|location=="Guinea"|location=="Nepal") %>% 
  ggplot(aes(x = mean_beds, y = mean_cases, colour = location,size=population)) + geom_point()+theme_bw()
...
covid$death_rate=(covid$new_deaths+1)/(covid$new_cases+1)
death_data<-covid %>% group_by(location)%>% summarise(death_mean=mean(death_rate))
death_data%>% filter(location=="Anguilla"|location=="Falkland Islands"|location=="Greenland"|location=="Laos"|location=="Timor"|location=="Turkey"|location=="Chile"|location=="Poland"|location=="Albania"|location=="Hondurus")%>%
  ggplot(aes(x=death_mean,y=location,color=location,size=5))+geom_point()+theme_bw()
